...
par(mfrow=c(1,2))
hist(d$y0,breaks=9,main="")
hist(d$y1,breaks=9,main="")
```

b. In a single paragraph, tell a story that could explain this distribution of treatment effects. **Ans:** One explanation is that there is that there is no relevant treatment effect on the visual accuity between kids who play outside at least 10 hours and the ones that play outside less than 10 hrs.  
c. What might cause some children to have different treatment effects than others? **Ans:** This might be just a measurement error.  
...
f. How different is the estimate from the truth?  Intuitively, why is there a difference? **Ans:** The difference with the true estimate is `r -0.04 + 0.06`. This is explained by the fact that we are no longer making a random selection on the treatment and control group so $E[Y_i(0)|D_i=1] - E[Y_i(0)|D_i=0]$ is no longer zero. This result can be explained by $E[Y_i(1)] - E[Y_i(0)] = E[Y_i(1) -Y_i(0)|D_i=1] + E[Y_i(0)|D_i=1] - E[Y_i(0)|D_i=0]$, where $E[Y_i(1) -Y_i(0)|D_i=1]=-0.18$ is the ATE among the treated and $E[Y_i(0)|D_i=1] - E[Y_i(0)|D_i=0]=0.06$ is the selection bias.  

To calculate  $E[Y_i(1) -Y_i(0)|D_i=1]=-0.18$ 
